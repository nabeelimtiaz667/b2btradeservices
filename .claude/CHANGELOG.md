...
- `buyer-detail.php`: added `csrf_field()` and the same hidden fields the working inline form already uses (`form_type=contact`, `source_page=buyer-detail`, `source_id=$inquiry['id']`, `lead_type=supplier`), plus an error-message div. Replaced the fake submit handler with a real `fetch()` POST to `contact/submit-ajax` (matching the JSON-response endpoint already used by the lead-capture popup and tradeshow/agent-partner modals), including `bumpCsrfToken` and real success/error branching instead of an unconditional fake success.
- `Contact::submitAjax()` never had the 2026-09-21 admin-notification/acknowledgement emails wired in at all (only `submit()` did). Added both calls, gated the same way `createLeadIfNew()` already is (skipped for `tradeshow_application`/`agent_partner_application`, since those aren't buyer/supplier contact leads) -- so this one change fixes the popup for every existing AJAX-based contact form that goes through this endpoint, not just this modal.
- **Verified live**: opened the "Contact Buyer" modal on a real inquiry page as a guest, submitted it, confirmed the request now actually hits `contact/submit-ajax` (200 OK), confirmed a new `contact_submissions` row lands with the correct `source_page`/`lead_type`, and confirmed both the admin-notification and sender-acknowledgement emails were attempted in the log (same code path as the already-working supplier flow). Test submission and its auto-created lead row deleted afterward.

## 2026-09-21 — Supplier/buyer "contact" form now emails admin + sends sender an acknowledgement
**Files:** `app/Helpers/email_helper.php`, `app/Controllers/Contact.php`
**Why:** owner asked me to first check what the contact form on supplier-profile/buyer-detail pages actually did -- found it only wrote to `contact_submissions` and (for unrecognized emails) created a new lead row, with zero emails sent to anyone (the existing `notifyAdminNewInquiry()` was wired only into the admin's own manual "add inquiry" screen, never into `Contact::submit()`). Owner then asked for two emails, reusing the site's existing SMTP/PHP-mail() mechanism: one to the admin with the full submission, one to the sender acknowledging receipt and naming which party (buyer/supplier) will reply.
- Added `notifyAdminOfContactSubmission(array $data): bool` to `email_helper.php` -- builds an HTML table of every non-empty submitted field and sends it via the existing `sendAdminNotification()` (so it already respects `admin_notification_email` and the `notify_on_inquiry` toggle on `admin/settings/email`, consistent with the other three admin notification types).
- Added `sendContactAcknowledgementEmail(string $toEmail, string $toName, ?string $recipientType): bool` -- same SMTP-with-PHP-mail()-fallback structure as `sendPasswordResetEmail()`/`sendLeadVerificationEmail()` (re-reads `smtp_*` settings fresh, tries `sendViaSmtp()`, falls back to `sendViaPhpMail()`, logs each path). Message says "The {recipientType} will get back to you shortly" when a type is known, else a generic "Our team will get back to you shortly."
- `Contact::submit()`: after a submission is saved, added `Contact::resolveContactedPartyType(string $sourcePage): ?string` (`supplier-profile`/`product-detail` -> `'Supplier'`, `buyer-detail` -> `'Buyer'`, everything else -> `null`, since the general contact page/package-inquiry/RFQ forms aren't "contacting" a specific party) and call both new functions. Scoped to `Contact::submit()` only -- `Contact::submitAjax()` (tradeshow/agent-partner/popup forms) was left untouched, out of what was asked.
- **Verified live** on the local copy: submitted both the supplier-profile form (as a guest) and the buyer-detail form, confirmed both land in `contact_submissions` with the right `source_page`/`lead_type`, confirmed the log shows both emails attempted in order (admin notification, then sender acknowledgement) and correctly falling back to PHP `mail()` since local SMTP is unconfigured (same expected local-only failure mode as every other email in this app -- nothing listens on port 25 here). Also confirmed `createLeadIfNew()`'s existing behavior still fires correctly alongside the new emails (a buyer lead from the supplier-profile form, a supplier lead from the buyer-detail form). All test submissions and the two test lead rows created during verification were deleted afterward.
- Note: `notify_on_inquiry` and `admin_notification_email` are both already configured on this local DB copy (imported from production), so on production this will start sending real emails to the currently-configured admin address the next time someone submits either contact form, once this file is deployed.

## 2026-09-20 — Content gate: relaxed free-tier threshold to page 2, guest "Register" opens lead popup instead
**Files:** `app/Controllers/BaseController.php`, `app/Controllers/Buyer.php`, `app/Controllers/Supplier.php`, `app/Controllers/Product.php`, `app/Views/partials/content-gate.php`, `public/assets/js/lead-popup.js`
**Why:** owner asked for two changes to the search-page content gate (buyer inquiries, suppliers, products): signed-up free-tier users should be able to browse page 2 as well (gate only from page 3 onward), while guests keep the existing page-2 gate; and guests hitting "Register" on the gate should get the popup lead-capture form instead of being sent to the full `/register` page.
- Added `BaseController::gateTierForPage(int $page, string $tier): ?string` -- guests gate after page 1 (unchanged), free-tier gates after page 2 (relaxed from page 1), privileged never gates. Replaced the previous inline `($page > 1 && $tier !== 'privileged') ? $tier : null` at all 8 call sites across `Buyer.php`, `Supplier.php`, `Product.php` with this one shared helper, so the three search modules can't drift out of sync with each other.
- `content-gate.php`'s guest branch: changed the "Register" `<a href="/register">` to a `<button data-open-lead-popup>`. Added a manual-open listener in `lead-popup.js` for `[data-open-lead-popup]` that reuses the existing modal/form/default-type-by-page logic, but (unlike the automatic nag triggers) bypasses the "already handled this session" suppression, since a deliberate click shouldn't be silently ignored just because an auto-popup fired earlier. The modal + its JS were already loaded site-wide for logged-out visitors via `partials/footer.php`, so no new include was needed.
- **Verified live** on the local copy, not just read: guest at `buyer?page_buyer=2` -- still gated (unchanged). Free-tier account (`alibaba@gmail.com`, temp password, restored after) at `page_buyer=2` -- real results now show; same account at `page_buyer=3` -- gated with the membership-upgrade message. Guest clicking "Register" on a gated page -- opens the "Join B2B Trade Services" popup with Supplier pre-selected (per the page's existing default-type rule), instead of navigating away.
- Note: the pager's real query param for a named group is `page_{group}` (e.g. `page_buyer`), not `page` -- caught this via the rendered pagination links while testing, not from assumption.

## 2026-09-16 — Buyer Inquiries: `resultsTotal` display offset by +1657
**Files:** `app/Views/pages/buyer-main.php`
**Why:** owner asked to add a fixed number on top of the real total shown to visitors on the public Buyer Inquiries listing page.
- Changed the "Showing X results out of Y" line to `$resultsTotal + 1657` instead of the raw `$resultsTotal` (a genuine `int` from `$pager->getTotal('buyer')` / `Buyer.php`). Cosmetic only -- doesn't touch pagination, the actual query, or `$inquiries` count.

## 2026-09-16 — Fixed production crash on `leads/detail/{uid}` for Starter-tier leads
**Files:** `app/Views/dashboard/admin/lead-detail.php`
**Why:** owner reported `https://b2btradeservices.com/leads/detail/S-002028` crashing (500) while every other lead's detail page worked fine. Traced via the production error log (`ErrorException: Undefined array key "starter"` at `lead-detail.php` line 86) plus local code reading.
- Root cause: the view's `$membershipColors` color-lookup array had a `'silver'` key that doesn't correspond to any real membership tier (`UserModel::getMembershipLevels()` only defines `free`, `starter`, `gold`, `platinum`, `vip`) -- `'silver'` was a mislabeled/legacy stand-in for `'starter'`. Any lead whose `membership_level` is `'starter'` (e.g. S-002028) hit an undefined-array-key error on line 86, fatal in production.
- Fix: renamed the `'silver'` key to `'starter'` (kept its original color `#adb5bd`) rather than adding a duplicate entry. No other file references `'silver'` as a membership tier.
- **Not yet deployed** -- production is updated by manual file copy, not git; owner still needs to copy this one file up.
- Also investigated the owner's separate "one user can't log in" report for the same user. `Auth::login()` (`app/Controllers/Auth.php`) has no logic branching on `membership_level` at all. Reset the password on the local scratch "Test" account (`test@test.com`, id 658, Starter tier) to a known value and logged in through the real UI end-to-end on the local dev copy -- succeeded cleanly, no error logged. Concluded the login report is very likely unrelated to this bug / not reproducible; left as-is per owner's instruction to monitor and re-report if it recurs.
- Added `.claude/launch.json` (points the sandboxed Browser tool at the existing local XAMPP Apache on port 80) so this login test could be done through an actual browser rather than code-reading alone. Local dev-tooling only, no secrets, not something that affects production.

## 2026-09-09 — Site-wide regression pass over the day's five security fixes; found BLOCKERS #31
**Files:** none (test-only pass; found BLOCKERS #31, no code changed)
**Why:** owner asked to test the site with all of today's changes (#28, #7, #8, #29, #9) applied together, beyond the targeted verification already done for each fix individually.
- Walked through the site as a real browser user/admin rather than curl alone: homepage, login, Manage Agents (add + delete), Manage Suppliers (add + delete, exercising the `supplier-form.php` CSRF fix specifically), Manage Buyer Inquiries (add + delete, the `inquiry-form.php` fix), CSV Import page (the `admin/import/index.php` fix), Manage Users (approve/reject/delete, the #9 role-check backstop), Site Settings, supplier dashboard (add + delete product), public registration, RFQ post, the lead-capture popup (AJAX + CSRF token rotation), and the contact form. Every flow succeeded with its expected flash/JSON message, confirmed against the database where relevant, and every disposable account/row/file created for testing was removed afterward (a full DB-wide sweep by email pattern at the end confirmed zero leftovers, including two unrelated stale rows from an earlier, prior session's testing that were cleaned up as found).
- **Found one real bug, logged as BLOCKERS #31**: `Auth::logout()`'s "You have been logged out successfully" flash message never renders on the resulting login page. Confirmed this is pre-existing, not caused by today's GET-to-POST route change (`git diff` shows `Auth::logout()`'s body untouched by that fix) -- isolated the likely cause to `$this->session->destroy()` being called *before* the `redirect()->with('success', ...)` that follows it, by noting that the *same* `redirect()->to('/login')->with('success', ...)` pattern (no preceding `destroy()`) worked correctly for registration in the same test session. Not fixed here -- flagged rather than patched blind, since the actual mechanism (does `destroy()` really prevent the following flashdata write from persisting, and is `regenerate()` a better fit here than `destroy()`) deserves a deliberate look, not a guess bolted onto an unrelated pass.
- Re-ran `php spark test:rate-limit` (20/20) and a full `php -l` sweep across every file touched today as a final gate -- all clean.

## 2026-09-09 — Fixed BLOCKERS #9: `AuthFilter` now actually applied, login is opt-out instead of opt-in
**Files:** `app/Config/Filters.php`
**Why:** owner asked to fix BLOCKERS #9, the last item in this session's ordered list -- `app/Filters/AuthFilter.php` was written and aliased as `'auth'` but never referenced anywhere in `Routes.php`, so every dashboard/admin controller method was individually responsible for its own `session()->get('logged_in')` check with no framework-level backstop. Worked today only because every method happened to remember it; a new one that forgot would be public by default.
- Added `'auth' => ['before' => ['dashboard', 'dashboard/*', 'admin/*']]` to `Filters::$filters` -- CI4's URI-pattern-based filter mechanism, not a `Routes.php` rewrite (all ~140 routes here are flat individual `$routes->get/post(...)` calls, no `$routes->group()` blocks, so this was far lower-risk than restructuring routing to attach a per-group filter). Listed the bare `dashboard` path separately from the `dashboard/*` wildcard because CI4's pseudo-regex for `*` requires a literal `/` after the prefix and won't match a path with nothing following it -- confirmed by reading `Filters::checkPseudoRegex()` before relying on it, not assumed.
- This doesn't replace any existing manual check -- `AuthFilter` only verifies `logged_in`, the same as every controller method already did, so the more specific role checks (`user_type !== 'admin'` etc.) each method does on top are untouched and still the thing that actually enforces *which* dashboard a given user can reach. This is purely the backstop the BLOCKERS entry asked for: a route under these two prefixes is now login-required unless explicitly excluded, rather than only protected if someone remembered to add the check.
- Confirmed via `grep` that no route under `dashboard/*` or `admin/*` is meant to be public -- every one belongs to `Dashboard` or `AdminSettings`, both authenticated-area controllers; no marketing/public page lives under either prefix.
- **Verified live, not just asserted:** unauthenticated `GET` to `/dashboard`, `/dashboard/agents`, and `/admin/settings` all redirect to `/login` (previously relied solely on the controller's own check, now backed by the filter too). Public routes (`/`, `/about-us`, `/product`, `/supplier`, `/buyer`, `/login`, `/register`, `/contact`) all still return `200` -- the URI-pattern scoping doesn't leak onto anything outside `dashboard/*`/`admin/*`. Logged in as a disposable admin: `dashboard/agents` and `admin/settings/general` both `200`. Logged in as a disposable buyer: `dashboard/agents` still redirects to `/login` (the controller's own role check still fires correctly -- the new filter didn't weaken or bypass it), while the buyer's own `dashboard/buyer` page still works. All test accounts removed afterward.
- Re-ran `php spark test:rate-limit` after this and all other 2026-09-09 changes (all 20 checks still pass) as a final regression sweep, since `Filters.php` had now been touched three times in one session (rate limiting, CSRF, this).

## 2026-09-09 — Fixed BLOCKERS #29: session ID now regenerates on login (session fixation)
**Files:** `app/Controllers/Auth.php`
**Why:** owner asked to fix BLOCKERS #29 -- `Auth::login()` set the authenticated session data (`$this->session->set($sessionData)`) without ever calling `regenerate()` first, so whatever session ID a visitor carried *before* logging in stayed valid *after* too. That's the exact precondition session fixation relies on: fix a victim's pre-auth session ID (a shared machine, a cookie-forcing vector), then inherit their authenticated session once they log in.
- Added `$this->session->regenerate(true)` immediately before `$sessionData` is set -- one line. `true` tells CI4 to actually destroy the old session's data (`session_regenerate_id(true)`) rather than leave an orphaned copy under the old id. Confirmed no other controller establishes an authenticated session (`grep -rn "'logged_in' => true"` across `app/Controllers` returns only this one site) -- registration and the lead-capture signup flow both redirect to `/login` rather than auto-authenticating, so this is the only place that needed the fix.
- **Verified live, not just asserted:** captured the session cookie before and after a real login -- the id changed (`93f2c533...` -> `cba93918...`). Checked `writable/session/` directly: the pre-login session's file was gone afterward, not just orphaned, confirming `regenerate(true)`'s destroy behavior actually took effect. Confirmed the new session still works for authenticated access (`/dashboard` -> 200 with the new cookie). Test account removed afterward.

## 2026-09-09 — Fixed BLOCKERS #8: 9 destructive GET routes converted to POST forms
**Files:** `app/Config/Routes.php`, `app/Views/dashboard/admin/agents.php`, `app/Views/dashboard/admin/inquiries.php`, `app/Views/dashboard/admin/submission-detail.php`, `app/Views/dashboard/admin/submissions.php`, `app/Views/dashboard/admin/suppliers.php`, `app/Views/dashboard/admin/users.php`, `app/Views/dashboard/buyer/inquiries.php`, `app/Views/layouts/dashboard.php`, `app/Views/partials/header.php`, `app/Views/partials/header-profile.php`, `app/Views/partials/header-inner.php`
**Why:** owner asked to fix BLOCKERS #8 -- 9 state-changing actions (`dashboard/delete`, `dashboard/approve`, `dashboard/reject`, agent/supplier/inquiry/submission/buyer-inquiry delete, and `logout`) were registered as `$routes->get(...)`, meaning any of them could fire from a plain link, an `<img src>`, a crawler, or a browser prefetcher on any page an authenticated admin happened to have open -- and, combined with #7 having just been closed, this was the next entry on the list the owner asked to work through in order.
- Changed all 9 routes in `Routes.php` from `->get(...)` to `->post(...)`. None of the 8 destructive controller methods (`Dashboard::approveUser/rejectUser/deleteUser/deleteAgent/deleteSupplier/deleteInquiry/deleteSubmission/buyerDeleteInquiry`) or `Auth::logout()` checked the HTTP method internally, so no controller changes were needed -- they simply become unreachable via GET now that the route itself only answers POST.
- Converted every `<a href="...">` triggering one of these into a `<form method="post">` with `<?= csrf_field() ?>` and a `<button type="submit">` carrying the exact same CSS classes the link had (`.btn-approve`/`.btn-reject`/`.btn-delete`/`.logout-btn`/`.md-logout`/etc. are all self-contained -- background, border, padding already declared -- so a `<button>` renders pixel-identical to the `<a>` it replaced). Kept every existing `onclick="return confirm(...)"` as the form's `onsubmit`. The 8 delete/approve/reject sites were one-off edits (5 files); the 9th action (`logout`) appears in 3 different link styles across 12 call sites (`layouts/dashboard.php` x2, and an identical plain-text pattern repeated 3x each in `header.php`/`header-profile.php`/`header-inner.php`) -- did the 9 identical ones via a scripted regex transform (confirmed byte-identical first) rather than 9 individual edits, and the 2 unique sidebar/mobile-drawer ones by hand, adding minimal inline style resets (`font: inherit; cursor: pointer;`, plus `border:none; background:none` only where the original class didn't already declare those) since native `<button>` chrome differs from an `<a>`'s.
- **Verified live, not just asserted:** all 9 routes now return `404` on a plain GET (previously all 9 executed the action). Logged in as a disposable admin and confirmed the new POST flows still work end-to-end: `logout` via POST succeeds and redirects to `/login`; created a disposable contact-submission row and deleted it through the new `dashboard/submissions/delete` POST form, confirmed the row is actually gone from the database afterward. Checked computed styles in a real browser session on `.btn-approve` and both logout buttons -- background/color/padding/border all match the original CSS exactly, and the DOM correctly shows `<button type="submit">` elements inside `<form>`s (not `<a>` tags) via the accessibility tree. All test data cleaned up afterward.

## 2026-09-09 — Fixed BLOCKERS #7: CSRF protection enabled site-wide, every form/AJAX call audited
**Files:** `app/Config/Filters.php`, `app/Filters/CsrfTokenHeader.php` (new), `public/assets/js/csrf-refresh.js` (new), `app/Views/admin/import/index.php`, `app/Views/dashboard/admin/inquiry-form.php`, `app/Views/dashboard/admin/supplier-form.php`, `app/Views/dashboard/admin/leads.php`, `app/Views/dashboard/admin/popup-leads.php`, `app/Views/partials/agent-partner-form-modal.php`, `app/Views/partials/apply-now-form.php`, `app/Views/partials/tradeshow-form-modal.php`, `app/Views/partials/footer.php`, `app/Views/layouts/dashboard.php`, `public/assets/js/homepage-lead-forms.js`, `public/assets/js/lead-popup.js`
**Why:** owner asked to fix BLOCKERS #7 -- CSRF was registered as a filter alias but commented out of `$globals`, so no route was protected (verified back on 2026-07-29 that an unauthenticated POST to `/contact/submit` was accepted with no token).
- Uncommented `'csrf'` in `Filters::$globals['before']`. Before doing that, audited every `<form>` in `app/Views` (58 files) and every raw AJAX POST call (grepped for `method:['"]post['"]`/`.ajax(` across `app/Views` and `public/assets/js`) against `csrf_field()` presence, to avoid the exact failure mode the original BLOCKERS entry warned about ("enabling it will break any form that doesn't send the token"). Found and fixed 3 real gaps: `admin/import/index.php`, `dashboard/admin/inquiry-form.php`, `dashboard/admin/supplier-form.php` -- all missing `<?= csrf_field() ?>` outright. Every other form already had it; the remaining 9 form-bearing files without it were confirmed to be GET-only search filters (safe, CSRF doesn't apply to GET) or `pages/rfq.php` (the dead, unrouted file from BLOCKERS #20).
- **Found and fixed a real correctness problem while wiring this up, not just "add the token and ship":** `Config\Security::$regenerate = true` means the CSRF hash rotates after *every* successful verification -- including ones where the controller then returns a business-logic error, not just successes. Any AJAX flow that can submit more than once without a full page reload (a validation-error retry, or the admin leads table's repeated inline stage/note edits) would work once, then throw a raw `SecurityException` on the second attempt with a stale embedded token. Confirmed this failure mode live before fixing it (see verification below), not just reasoned about it.
  - Added `app/Filters/CsrfTokenHeader.php`, a global `after` filter that echoes the current (already-rotated) hash back as an `X-CSRF-TOKEN` response header on every request -- cheap (one header read), and means no future AJAX endpoint needs remembering to wire this up individually.
  - Added `public/assets/js/csrf-refresh.js` (`bumpCsrfToken()`), loaded sitewide (`footer.php` for public pages, `layouts/dashboard.php` for admin), which reads that header off a `fetch()` response and updates every `csrf_test_name` hidden field on the page, plus a `window.__csrfTokenValue` fallback for the two admin screens (`leads.php`, `popup-leads.php`) that build POST bodies by hand instead of submitting a `<form>`.
  - Piped all 7 AJAX call sites (`homepage-lead-forms.js`, `lead-popup.js`, `agent-partner-form-modal.php`, `apply-now-form.php`, `tradeshow-form-modal.php`, and the 4 raw-fetch endpoints in `leads.php`/`popup-leads.php`) through `bumpCsrfToken`.
- **Verified live, not just asserted:**
  - No-token POST to `/contact/submit` now returns `403` (was previously accepted, per the original BLOCKERS finding).
  - A real form submission with a valid token still succeeds end-to-end (`/contact/submit` -> 303 -> success flash -> row inserted; cleaned up after).
  - `leads/ajax-update-stage` called twice in a row in a real browser session, refreshing the token via `bumpCsrfToken` between calls: both succeed (200/`success:true`). As a control, repeated the same two calls *without* refreshing the token: the second fails with `403`/`SecurityException` -- proving the fix is both necessary and effective, not coincidentally working. (Real lead record's stage and activity-log entries restored to their pre-test state afterward.)
  - `leads/popup/add-note` and `leads/popup/update-inline` both work with a valid token (test note/log entries cleaned up).
  - Combined with the #28 upload fix below: a multipart product-upload form with a valid CSRF token still correctly rejects a malicious file and correctly accepts a legitimate one -- confirming the two fixes compose correctly together, not just individually.

## 2026-09-09 — Fixed BLOCKERS #28: unrestricted file upload → remote code execution
**Files:** `app/Helpers/upload_helper.php` (new), `app/Config/Autoload.php`, `app/Controllers/Dashboard.php`, `public/uploads/.htaccess` (new), `.gitignore`
**Why:** owner asked to fix BLOCKERS #28, found in a full-app security audit the same day -- every image upload (`main_image`, `company_logo`, banners, inquiry attachments) called only `isValid()`/`hasMoved()` before `move()`, no file-type check anywhere. A `.php` file whose real (finfo-detected) MIME type is `text/x-php` gets mapped straight back to a `.php` extension by `Mimes.php`, and lands executable in the web-served `uploads/` folder. Chain required nothing but a self-registration (open by default) and a supplier product upload -- no admin approval needed, since `Auth::login` never gated on `status` and the upload guard only checked `user_type`.
- **First, actually verified the audit's own claims before building the fix** (per CLAUDE.md's "verify before claiming"): traced `Mimes.php`'s `text/x-php` -> `php` mapping, confirmed `public/uploads/` had no `.htaccess` at all, confirmed `UploadedFile::getMimeType()` reads real file content via `finfo_file()` (not the client's `Content-Type` header, which is what makes it trustworthy to check against). This also surfaced a correction to the original audit: of the ~16 `move()` call sites in `Dashboard.php`/`AdminSettings.php`, only the 4 `main_image` (product image) sites had *zero* pre-existing validation -- company logo, banner and attachment uploads already had a `getMimeType()` allowlist check before `move()` (just missing the `getimagesize()` decode confirmation), so they were not actually exploitable via this specific chain, contrary to the audit's blanket "every affected upload site" framing. `AdminSettings.php`'s hero-banner upload was already fully guarded (MIME + `getimagesize()` + dimensions) and needed no change.
- Added `validate_uploaded_image()` to a new `app/Helpers/upload_helper.php` (autoloaded via `Autoload.php`, matching the existing `check_restricted_keywords()` shared-helper pattern rather than copy-pasting): checks the file's real `getMimeType()` against an image allowlist, then confirms `getimagesize()` can actually decode it, before anything is allowed to call `getRandomName()`/`move()`.
- Inserted a `validate_uploaded_image()` call (return an error and abort the request if it fails) into all 14 upload guards in `Dashboard.php` via a scripted transform (all 14 shared the identical `if ($x && $x->isValid() && !$x->hasMoved())` guard shape, confirmed by grep before touching anything) -- redundant on the 10 sites that already had their own `getMimeType()` check, load-bearing on the 4 `main_image` sites that had none.
- Added `public/uploads/.htaccess` as a second, independent layer: disables PHP execution (`php_flag engine off` for this box's `php_module`, plus `mod_php.c`/`mod_php7.c` for other Apache builds; `RemoveHandler`/`RemoveType` for cPanel-style handler setups; a `<FilesMatch>` deny as a third fallback) so a future validation gap still can't execute. Discovered and fixed a deploy gap while adding this: `public/uploads/` is entirely gitignored (DECISIONS #10, ~95MB of media), which would have silently prevented this file from ever reaching production -- added a `/public/uploads/*` + `!/public/uploads/.htaccess` exception to `.gitignore` (had to switch the parent rule from `/public/uploads/` to `/public/uploads/*` first -- git won't re-include a file inside a directory excluded outright, only inside one whose *contents* are excluded). Verified: `git status`/`git add --dry-run` now picks up `.htaccess` while real uploaded media stays ignored.
- **Verified live, not just asserted:** created a disposable approved supplier account, uploaded a real `<?php echo "SHOULD_NEVER_UPLOAD"; ?>` file as a product's `main_image` -- rejected with "Only JPG, PNG, GIF and WEBP images are allowed.", no file created, no product row inserted. Separately placed a benign PHP file directly into `public/uploads/products/` and requested it over HTTP: `403 Forbidden`, not executed (confirmed no output, not just a non-200 status). Confirmed a real image upload through the same form still succeeds normally. All test accounts, files and rows removed afterward.
...
- Replaced it with a project-specific setup guide: requirements, what's deliberately excluded from `git clone` and why (`vendor/`, `.env`, `blog/`, `sql/*.sql` dumps, `app/Data/countries.php`/`rate_limits.php`, uploaded/product images — each with a one-line reason and where to get it), then numbered setup steps (install deps, `.env` from `env`, database create + restore-or-migrate, `countries:sync`, web server config, verify with a `curl` check), plus a "before you configure anything else" section repeating the SMTP/PII warnings from CONTEXT.md so they're not missed by someone who only reads the README. Ends with links to the `.claude/` docs for everything not setup-related, rather than duplicating them.
- **Actually tested every step while writing it**, rather than transcribing CONTEXT.md's existing "Commands" section (which just says `composer install` unqualified) — and found `composer install` genuinely fails from a truly fresh clone on this box's PHP 8.1.5: `laminas/laminas-escaper`, one of only two real runtime dependencies, is locked in `composer.lock` to a version requiring PHP 8.2+. Confirmed via `composer install --no-dev --dry-run` (fails), `composer update laminas/laminas-escaper --no-dev --dry-run` (fails on `phpunit`/`php-cs-fixer` instead — `--no-dev` doesn't skip resolving `require-dev` during `update`, only skips installing it), and `vendor/laminas/laminas-escaper/composer.json`'s own installed copy declaring `"php": "8.2.99"` — meaning this box's `vendor/` was populated elsewhere on PHP 8.2+ before `composer.lock` drifted further, and keeps working here only because PHP doesn't enforce a package's declared version constraint at runtime (only Composer's install step does). Logged as **BLOCKERS #27** (HIGH) with the full diagnosis and the pragmatic workaround (copy an already-populated `vendor/` from a working machine rather than reinstalling) documented in the new README. All checks were read-only — `composer.json`/`composer.lock`/`vendor/` are unchanged, confirmed by diffing against backups taken before each dry-run.
- This is the same underlying `composer.lock`-vs-PHP-8.1 drift flagged in the 2026-09-05 CHANGELOG entry (which was about `phpunit`/`php-cs-fixer`, the dev tooling, and was declined in favor of a non-PHPUnit test script) — today's finding shows it's broader than dev tooling and now blocks a fresh install outright, not just `composer require --dev phpunit/phpunit`.

## 2026-09-06 — Gitignored `app/Data/countries.php` and `app/Data/rate_limits.php`, resolving BLOCKERS #26
**Files:** `.gitignore`, `app/Data/countries.php` (untracked, not deleted), `app/Data/rate_limits.php` (untracked, not deleted), `.claude/BLOCKERS.md`
**Why:** owner asked directly whether BLOCKERS.md was being kept current, which surfaced that neither of these two runtime-writable files (rewritten by "Update Now" and the Site Security page, respectively) had been flagged as a deploy risk despite being git-tracked -- logged as #26, then the owner asked to just fix it outright rather than leave it open.
- Added both paths to `.gitignore`, then `git rm --cached` on both -- removes them from git's tracking without touching the files on disk (verified: both still present locally with their real content immediately after, `php spark test:rate-limit` still passes all 20 checks).
- This is the same tradeoff BLOCKERS #25 already describes for `public/assets/images/`/`public/uploads/` (git status/diff going blind to a path), taken deliberately here so a future manual deploy-file-copy can never silently overwrite production's live admin-set values (a real country sync, a real rate-limit change) with whatever's committed in this repo. Noted in #25 itself, not just #26, since it's now the same standing risk for two more paths -- any future deploy checklist needs to know about `app/Data/countries.php` and `app/Data/rate_limits.php` too.
- Consequence worth knowing: a *fresh* clone of this repo will no longer have real country data out of the box -- `php spark countries:sync` needs to run once. `rate_limits.php`'s absence is harmless by design (`RateLimitFilter::overrides()` already treats a missing file the same as an empty one, falling back to each form's own default).
- Resolved BLOCKERS #26 into #25 rather than leaving it as its own closed-but-separate entry, since it's genuinely the same risk on the same mechanism, not a distinct one.

## 2026-09-06 — Fixed step-1 lead-capture form: Supplier/Buyer toggle and button left-aligned instead of centered (popup + homepage)
**Files:** `public/assets/css/style.css`, `app/Views/partials/lead-popup-modal.php`
**Why:** owner reported the popup's and homepage's step-1 lead form (Join B2B Trade Services / Get Multiple Quotes / Find us -- all render `partials/lead-capture-inline-form.php`) had the Supplier/Buyer radio row sitting left instead of centered, and the "Get Started" button capped at a fixed width instead of filling the card -- while the same form in the footer looked correct. Traced it (not a regression from this session's work -- `git log` shows nothing this session touched these files or their CSS, and none of today's other commits reference `.filter-group`, `.gradeint-cta`, `.multiple-quote-form`, or `#leadPopupModal`) to the footer's `.b2b-top-form` wrapper being the *only* one that ever had the two overrides needed: `.b2b-top-form .filter-group { justify-content: center; }` and `.b2b-top-form .gradeint-cta { max-width: 100% !important; }`. The homepage's `.multiple-quote-form` wrapper and `#leadPopupModal` never had the equivalent rules, so both fell back to the base (left-aligned, 212px-capped) `.filter-group`/`.gradeint-cta` styles that the rest of the site's other, unrelated buttons/toggles also use.
- Added `.multiple-quote-form .filter-group { justify-content: center; }` and `.multiple-quote-form .gradeint-cta { max-width: 100%; }` to `style.css`, scoped to that one wrapper class rather than touching the base `.filter-group`/`.gradeint-cta` rules those two other selectors reuse sitewide. Confirmed `.multiple-quote-form` is used on other pages too (product detail's RFQ form, etc.) but none of those instances contain a `.filter-group` or `.gradeint-cta` inside them (grepped every page using that wrapper), so this is a no-op everywhere except the three homepage lead-capture instances ("Get Multiple Quotes" x2, "Find us") it's meant to fix.
- Added the equivalent `#leadPopupModal .filter-group` / `#leadPopupModal .gradeint-cta` overrides inside that partial's own existing scoped `<style>` block -- an ID selector, so this can't affect anything outside that one modal.
- Verified live: forced the popup open and screenshotted it (radio row centered, button full-width, matching the sidebar's already-correct copy of the same form); for the homepage, read `getComputedStyle()`/`getBoundingClientRect()` on all three `.multiple-quote-form` instances after login as a guest, confirming `justify-content: center` and a button width matching the card (not the old 212px cap) on each. No unrelated page's `.filter-group`/`.gradeint-cta` usage was touched (verified by grep -- no other page mixes those classes with `.multiple-quote-form`).

## 2026-09-05 — Rate-limit overrides moved from `site_settings` (DB) to a plain PHP file
**Files:** `app/Filters/RateLimitFilter.php`, `app/Controllers/Dashboard.php`, `app/Data/rate_limits.php` (new), `app/Commands/TestRateLimit.php`
**Why:** owner asked, correctly, whether storing the admin-editable "N" in the database meant every hit to a rate-limited form -- including the ones a brute-force attempt makes *on purpose* to get blocked -- triggered a `SELECT` first. It did (`SiteSettingModel::getSetting()` has no caching, so `RateLimitFilter::before()` ran one query per request before the Throttler even got a chance to reject it, backwards from the point of rate limiting a DB-heavy form like login). Owner then suggested the fix themselves: since an admin changes this rarely, mirror `app/Data/countries.php`'s pattern -- a plain PHP array file instead of a table.
- `RateLimitFilter` no longer imports or touches `SiteSettingModel` at all. Overrides now come from `RateLimitFilter::overrides()`, a plain `require` of `app/Data/rate_limits.php` (name => capacity). Deliberately not cached in a static/local variable across requests either: under PHP-FPM a worker serves many requests over its life, and a process-lifetime cache would mean an admin's save silently doesn't take effect until that worker recycles. A bare `require` is already cheap, and PHP's opcache revalidates the compiled file against its mtime by default, so a save is live on (essentially) the next request.
- Added `RateLimitFilter::saveOverrides()`: same temp-file-plus-`rename()` atomic write `CountrySyncer::writeAtomic()` uses, so a concurrent reader never sees a half-written file. `Dashboard::updateSiteSecurity()` now calls this instead of nine `SiteSettingModel::setSetting()` calls.
- Shipped `app/Data/rate_limits.php` checked in with `return [];` (like a fresh `countries.php` would ship with real data) -- empty means "use each form's own default", populated only once an admin saves the Site Security page for the first time.
- `TestRateLimit` gained a fourth test (`testAdminOverrideFile`) proving the write/read round-trip and that `RateLimitFilter::before()` actually honors an override -- backs up the real file's bytes first and restores them byte-for-byte in a `finally`, even if an assertion fails, since this file is live config, not test scratch space. Also de-duplicated `testConfiguredRouteLimits`'s hardcoded expectations to read from `RateLimitFilter::forms()` instead of keeping a third hand-maintained copy of the same numbers.
- Verified: `php spark test:rate-limit` passes all 20 checks (16 before, plus the new file-round-trip test's 3 assertions and one added cross-check), and `grep` confirms zero references to `SiteSettingModel`/`Model`/`Database` anywhere in `RateLimitFilter.php`.

## 2026-09-05 — New admin page: Site Security (admin-editable rate-limit "N" per form)
**Files:** `app/Filters/RateLimitFilter.php`, `app/Controllers/Dashboard.php`, `app/Config/Routes.php`, `app/Views/dashboard/admin/security.php` (new), `app/Views/layouts/dashboard.php`
**Why:** owner wants to adjust how many submissions ("N") each rate-limited public form allows without a deploy -- the limits from the same-day rate-limiting change were hardcoded in `Routes.php`'s filter arguments.
- Added `RateLimitFilter::forms()`: a static array that's now the single source of truth for every rate-limited form's name, admin-facing label, fixed time window, and default N. `Dashboard::siteSecurity()`, `TestRateLimit`'s drift check, and the filter itself all read from (or are checked against) this one place instead of three independently-hand-kept lists.
- `RateLimitFilter::before()` now looks up a `site_settings` row (`ratelimit_{name}`) before falling back to the route's own default N; the time window stays fixed and is not stored in the DB (not asked for, and changing a window retroactively vs. mid-bucket has odd edge cases N doesn't). An override of `0` or blank is ignored (falls back to default) so an admin can't accidentally zero-out a form.
- New `dashboard/security` page (`Dashboard::siteSecurity()` / `updateSiteSecurity()`, same inline session-guard convention as `countries()`): one row per form showing its label, fixed window, and an editable N (clamped server-side to 1-1000 on save via `SiteSettingModel::setSetting()`, group `'security'`). Added as its own link ("Site Security", shield icon) under the Administration group in both the desktop sidebar and mobile drawer, right after "Countries" -- not nested under Site Settings, matching how Countries was placed.
- Verified live with a disposable admin test account: page renders all 9 forms with their correct current values; saving `ratelimit_login=3` persisted, redisplayed correctly, and a real HTTP test against `/login` confirmed the 4th rapid attempt is now blocked (was 8 before); restored the setting to 8 and deleted the test `site_settings` rows and account afterward so the page starts from code defaults again for the next real admin.

## 2026-09-05 — Repeatable test command for rate limiting: `php spark test:rate-limit`
**Files:** `app/Commands/TestRateLimit.php` (new)
**Why:** owner asked for a real test in the project (not another throwaway QA command deleted after use) so they can re-verify the rate limiting themselves at any time, e.g. after touching `Routes.php` or the filter.
- Tried to install PHPUnit first (already a `require-dev` in `composer.json` but never actually installed — `vendor/` had no dev packages at all). Both a plain `composer install` and a scoped `composer require --dev phpunit/phpunit` failed: the committed `composer.lock` pins several dev tools (`friendsofphp/php-cs-fixer`, `symfony/console`, `laminas/laminas-escaper`, etc.) to versions requiring PHP 8.2+/8.4, while this box runs 8.1.5. Fixing that needs a full `composer update` across all of `require-dev`, which the owner decided not to do right now — so this is a plain `php spark` command instead, consistent with how every other check in this project already runs, not a PHPUnit test class under `tests/`.
- `TestRateLimit` calls the real `CodeIgniter\Throttle\Throttler` service directly (same one `RateLimitFilter` uses) with unique, disposable per-run keys, so it never touches a real visitor's bucket and leaves nothing behind (`$throttler->remove()` after every check). Four checks: exact capacity is enforced (N allowed, N+1th blocked), two different bucket keys never share state, a bucket refills and allows again once its window fully elapses (proves it's a rolling window, not a permanent lockout), and — the one that catches drift — every route's actual `'filter' => 'ratelimit:name,capacity,seconds'` string in `Routes.php` is cross-checked against the limits this project documents, so editing a limit in one place without the other fails loudly instead of silently.
- Verified the drift check actually catches drift, not just passing by construction: temporarily changed `login`'s route filter to `ratelimit:login,99,60`, confirmed the command printed `[FAIL]` and exited non-zero, then restored the original line and confirmed a clean pass again (`git diff` showed only this session's real rate-limiting changes, nothing left over from the test edit).
- Run it any time with `php spark test:rate-limit`; takes about 3 seconds (one deliberate `sleep()` to prove refill-over-time). Exit code is 0 on all-pass, 1 if anything fails, so it's scriptable.

## 2026-09-05 — Rate limiting on public-facing forms (login, register, password reset, contact, RFQ, lead capture)
**Files:** `app/Filters/RateLimitFilter.php` (new), `app/Config/Filters.php`, `app/Config/Routes.php`
**Why:** owner asked for rate limiting on the forms the app accepts submissions through, to blunt brute-force login attempts, registration/contact spam, and email-bombing via forgot-password. Scoped to public/anonymous-reachable forms only, not authenticated admin/dashboard CRUD (a compromised admin session is a different problem, and rate-limiting trusted internal workflows would just add friction).
- New `RateLimitFilter` (alias `ratelimit`) wraps CI4's built-in `Throttler` service (token-bucket, file-cache backed -- same cache driver already used everywhere else, no new dependency). Route filter argument is `name,capacity,seconds`, e.g. `ratelimit:login,8,60` = 8 submissions per 60 seconds per IP; each name is its own bucket so forms never share a counter.
- Bucket key hashes the client IP (`md5()`) before use -- CI4 cache keys reject `{}()/\@:`, which a raw IPv6 address (`::1` on local dev) contains, so building the key from the address directly throws.
- Applied to: `login` (8/60s), `register` (5/300s), `forgot-password` (3/300s -- tightest, since it triggers an email send), `reset-password/{token}` (5/300s), `contact/submit` and `buyer/post-rfq` (5/300s each, separate buckets despite sharing `Contact::submit`), `contact/submit-ajax` (5/300s), `lead/capture` (8/300s), `lead/complete/{token}` (5/300s). Left alone: every `dashboard/*`, `admin/*` and `leads/*` route (all already behind a login/role filter).
- On block: redirect-based forms get `redirect()->back()->withInput()->with('error', ...)`, the same flash-message pattern already used for validation errors on these pages. AJAX forms (`contact/submit-ajax`, `lead/capture`) get a `429` with a JSON body in the exact `{status, message}` shape their own success/error responses already use, detected via `$request->isAJAX()` -- confirmed every AJAX form in the codebase sends `X-Requested-With: XMLHttpRequest` and unconditionally calls `.json()` on the fetch response regardless of status code, so this renders as an ordinary inline form error, not a broken request.
- Verified: a direct unit test of the `Throttler` (bypassing HTTP round-trip latency) confirmed exactly 8 of 10 rapid-fire calls allowed and the rest blocked; live HTTP tests against `contact/submit-ajax` (pre-exhausted bucket) returned a real `429` with the friendly JSON message, and against `register` (pre-exhausted bucket) returned the redirect + flash message. Normal, non-throttled submissions on `login` and `register` still work.

## 2026-09-05 — Countries & Phone Codes page moved: Site Settings tab → own link under Administration
**Files:** `app/Controllers/AdminSettings.php`, `app/Controllers/Dashboard.php`, `app/Config/Routes.php`, `app/Views/dashboard/admin/countries.php` (new), `app/Views/admin/settings/countries.php` (deleted), the "Countries" nav-tab line removed from the other 9 `admin/settings/*.php` views, `app/Views/layouts/dashboard.php`
**Why:** owner explicitly asked for this page to not be a sub-tab of Site Settings, but its own top-level link in the admin sidebar's "Administration" group (alongside Manage Users, Manage Suppliers, Site Settings, etc.), matching how the rest of that group works.
- Moved `countries()` / `syncCountriesNow()` from `AdminSettings.php` to `Dashboard.php`, switching from `AdminSettings`'s `checkAdmin()` helper to `Dashboard.php`'s own inline `logged_in`/`user_type` guard, to match that file's existing convention.
- Routes changed from `admin/settings/countries` (+ `/sync`) to `dashboard/countries` (+ `/sync`); the old URLs now 404 (confirmed live).
- View moved from the Site-Settings tab-nav shape (`admin/settings/countries.php`) to the plain `dashboard/admin/*` page-header shape (`dashboard/admin/countries.php`), consistent with every other Administration-group page. Reverted the "Countries" tab-link mechanically added to the other 9 Site-Settings views in the previous change.
- Added a "Countries" link (globe icon) to both the desktop sidebar and mobile drawer's Administration block in `layouts/dashboard.php`, right after "Site Settings", with the same active-state `preg_match` pattern used by every other nav entry.
- Verified live end-to-end with a disposable admin test account: link appears in both desktop and mobile nav under Administration, gets the `active` class on `/dashboard/countries`, the page itself renders the country table and correct "Last updated" timestamp, and the old `/admin/settings/countries` URL now 404s. Test account deleted afterward.

## 2026-09-05 — New admin page: Countries & Phone Codes (list + "Update Now")
**Files:** `app/Libraries/CountrySyncer.php` (new), `app/Commands/CountriesSync.php` (simplified), `app/Controllers/AdminSettings.php`, `app/Config/Routes.php`, `app/Views/admin/settings/countries.php` (new), plus one nav-tab line added to the other 9 `admin/settings/*.php` views
**Why:** owner wants a way to see the current country/phone-code list and its last-sync date from the admin UI, with a manual refresh button, instead of only via `php spark countries:sync` on the CLI.
- Extracted the actual sync logic (fetch from `mledoze/countries`, preserve existing DB ids, write `app/Data/countries.php` atomically) out of the `CountriesSync` spark command into a new `CountrySyncer` library class, callable from anywhere. The command is now a thin wrapper around it; a new `AdminSettings::syncCountriesNow()` web action calls the exact same class, so the CLI and the "Update Now" button can never drift apart.
- New `admin/settings/countries` page: a searchable table of every country (flag, name, code, phone code, region) from `CountryModel::findAll()`, plus "Last updated: {date}" read from `app/Data/countries.php`'s own file-modified time -- accurate regardless of whether the last sync ran from the button or a cron'd CLI call.
- "Update Now" button posts to `admin/settings/countries/sync`, guarded by a JS `confirm()` dialog (declining does not submit) and a disabled/"Updating..." button state while the real request is in flight; on completion it redirects back with a flash message reporting how many countries were synced.
- Added a "Countries" tab to the Site Settings nav on all 10 `admin/settings/*` pages (this one included) for consistent navigation -- that nav list is duplicated per-file in this codebase already (not a shared partial), so this follows the existing pattern rather than refactoring it.
- Verified live: page loads with the current 250-country list and correct last-updated timestamp; the search box filters rows correctly; declining the confirm dialog does not submit; accepting it shows the loading state and, on the real request, actually re-runs the sync -- confirmed via the "Last updated" timestamp advancing and the flash message reporting "Synced 250 countries (128 new)."; confirmed a guest gets redirected to `/login` (same `checkAdmin()` guard as every other admin/settings page).

## 2026-09-04 — Pagination paywall: page 2+ of every public listing gated by sign-in / membership
**Files:** `app/Controllers/BaseController.php`, `app/Controllers/Product.php`, `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`, `app/Views/partials/content-gate.php` (new), `app/Views/pages/product.php`, `app/Views/pages/supplier.php`, `app/Views/pages/supplier-category.php`, `app/Views/pages/supplier-country.php`, `app/Views/pages/buyer-main.php`, `public/assets/css/style.css`
**Why:** owner wants page 1 of every listing (Products, Suppliers, Buyer Inquiries — both plain browse and search-result pages) to stay fully open, but page 2 onward gated: guests get pushed to sign in/register, logged-in free-tier users get pushed to buy a membership, admins and paid members see everything. A soft paywall on pagination depth, not on the data itself.
- Added `BaseController::contentAccessTier()` (`'guest'`|`'free'`|`'privileged'`), generalizing what was `Buyer::canViewPremiumDetails()`'s tier check (buyer contact-detail masking on `buyer-detail.php`) into a shared method every controller can use. `canViewPremiumDetails()` is now a one-line wrapper around it (`=== 'privileged'`) instead of a second copy of the same logic. Checked live against the DB every time, never trusted from session data (`membership_level` is never written into the session — see the method's docblock).
- All 9 paginated listing methods (`Product::index/bySupplier/search`, `Supplier::index/category/country/search`, `Buyer::index/search`) now compute `$data['gateTier']` = the tier, but only when the current page is >1 and the tier isn't `'privileged'` — otherwise `null`. The underlying query/pagination logic is completely unchanged; only what gets rendered changes.
- New shared partial `partials/content-gate.php` replaces the real results grid/list when `$gateTier` is set: generic skeleton/shimmer placeholder cards (no real product/supplier/inquiry data anywhere in the output — confirmed by grepping the response HTML, not just visually hiding it) plus an overlay -- `guest` gets "Sign In" + "Register" buttons (to `/login` and `/register`, not the popup CTA), `free` gets a single "View Membership Plans" button (to `/premium-services`). Pagination controls, the "Showing X out of Y" count, and page 1 itself are completely unaffected -- clicking through to page 2 is what triggers the gate, not a disabled link.
- Verified live end-to-end across all 9 methods with `fetch()` + regex checks on the raw response body (not just what's visually shown): guest page 1 has real detail-page links, guest page 2+ has the gate markup and *zero* real links; a free-tier test account gets the single-button gate; an admin test account gets full, ungated page 2 content; re-verified `canViewPremiumDetails()`'s existing buyer-contact-detail mask still works correctly for a free-tier user after the refactor. All test accounts deleted afterward.
- Follow-up verification (same day): the first pass only checked one paid tier (`gold`). Owner asked whether all paid tiers were actually tested rather than assumed from the code's `in_array(..., ['starter','gold','platinum','vip'])` check -- they hadn't been, so created one test account per tier (`free`/`starter`/`gold`/`platinum`/`vip`) and re-ran the same page-2 checks against `/product`, `/supplier`, and `/buyer/search` for each. Confirmed: `free` gated on all three, `starter`/`gold`/`platinum`/`vip` all fully ungated on all three. Test accounts deleted afterward.

## 2026-09-03 — Country system revamped: 122 manual countries → 250 from a free dataset, file-backed
**Files:** `app/Commands/CountriesSync.php` (new), `app/Data/countries.php` (new, generated), `app/Models/CountryModel.php` (rewritten), `app/Models/UserModel.php`, `app/Controllers/Dashboard.php`, `app/Views/pages/register.php`, `app/Views/pages/lead-complete-signup.php`, plus 10 view files' flag-rendering (see below)
**Why:** the `countries` table was manually entered with only 122 countries, so users from the rest of the world couldn't register or be found by country search. See `.claude/DECISIONS.md` #15 for the full reasoning (no FK constraints, one SQL join, no admin UI, and why `mledoze/countries` replaced the originally-planned `restcountries.com` after that API's free tier turned out to require an account).
- `php spark countries:sync` fetches `mledoze/countries` (GitHub raw, with a jsDelivr fallback), builds `app/Data/countries.php` (`name`, `code`, `phone_code`, `flag` as a `flagcdn.com` URL, `region`, `status`), preserving every existing country's id from the live DB table so no `country_id` foreign key anywhere needs to change. Writes atomically (temp file + `rename()`); aborts without touching the existing file if the fetch fails or returns a suspiciously short list (<190).
- Caught and fixed a real bug while building this: the source data's `idd.suffixes` lists every member territory's area code for shared-dial-code zones (US alone has 300+), not a single country suffix -- naively doing `root + suffixes[0]` produced `+1201` for the US instead of `+1`. Fixed: only append the suffix when there's exactly one (the normal case); shared-code countries use the root alone.
- `CountryModel` no longer extends CI4's `Model` / touches the DB -- it's a plain PHP class reading the generated file (cached in a static property per request), keeping the same method names (`find`, `findAll`, `getActiveCountries`, `getCountryByCode`, `getCountriesByRegion`) so no other call site needed to change. Added `getCountriesByCodes()` to replace the one raw `whereIn('code', $codes)->findAll()` chain (`UserModel::getLeads()`'s region filter), which doesn't exist over a plain array.
- `Dashboard::suppliers()`'s `sort=country_name` branch (the app's only SQL `JOIN` touching `countries`) now fetches all suppliers, enriches + sorts by country name in PHP, and paginates manually via `Pager::store()` -- the same primitive `Model::paginate()` uses internally, so the view's existing `$pager->links(...)` call needed no changes. Verified live: ascending/descending sort and page 2 all correct.
- Standardized flag rendering across every view that had it (`pages/supplier.php` x2, `supplier-profile.php`, `supplier-country.php` x2, `supplier-category.php`, `product-detail.php`, `buyer-main.php`, `buyer-detail.php` x2, `index.php` x3, `dashboard/admin/leads.php`, `dashboard/admin/inquiries.php`) onto one convention: `esc($country['flag'])` directly (already a full `flagcdn.com` URL), replacing three previously-inconsistent patterns -- most used the stored filename directly, two rebuilt a filename from the country *name* slug instead, and one had a `??`-operator precedence bug that silently changed behavior depending on whether `flag` was set.
- Cross-linked the "Select Country" dropdown with the `intl-tel-input` phone widget on the two forms where both appear (`register.php`, `lead-complete-signup.php`): selecting a country now calls `iti.setCountry()` to default the phone dial code to match, instead of the two being independent pickers that could disagree. `intl-tel-input` itself is unchanged (still handles phone formatting/validation) -- only the country data feeding it changed. The other 4 places `intl-tel-input` is used (admin edit-user/agents use plain text `phone_code` inputs with no widget; the popup-lead-capture forms/JS have no country field at all) needed no change.
- Verified live end-to-end: registration page lists all 250 countries with working country→phone-code sync; `supplier/search/.../country/CN` still resolves and filters correctly (72 results, same as before); flag images render from `flagcdn.com` on the homepage, supplier pages, buyer inquiries, and admin leads with zero old-style broken references left; a full page sweep (homepage, register, supplier listing/search/country/category, buyers, product, leads, admin suppliers, admin inquiries) came back with no PHP errors.
- The DB `countries` table (122 rows) was deliberately left in place, unread, rather than dropped -- no migration risk for no benefit. `public/assets/images/flags/` (the old locally-hosted flag files) is similarly left in place but now unreferenced.

## 2026-09-03 — Swept the codebase for the same incomplete-status-dropdown pattern
**Files:** `app/Views/dashboard/admin/inquiries.php`
**Why:** after fixing the two inquiry-status dropdowns (entry below), checked every other `<select name="status">` and status-driven filter dropdown in the codebase for the same bug: an `<option>` list missing a real status value, causing a silent fallback to the first option.
- Checked all 12 files with a `name="status"` select (popup leads list/edit, categories, supplier add/edit, agents, edit-user, users filter, contact submissions list/detail, plus the two already fixed) against each field's actual whitelist/valid-value set. All were already complete except the one fixed below -- categories (active/inactive), users/agents/suppliers (approved/pending/rejected), popup leads (its 3-value status and 8-value stage), and contact submissions (new/read/replied/closed) all matched their real value sets exactly.
- Checked the admin product-for-supplier forms (`adminAddProductForSupplier`/`adminEditProductForSupplier`) -- neither has a status field in its form at all (add hardcodes `active`; edit never touches status), so this bug class doesn't apply there.
- Found one more real instance: the "Manage Buyer Inquiries" page's client-side Status *filter* dropdown (`#filterStatus` in `dashboard/admin/inquiries.php`) only listed `active`/`closed`/`expired` -- missing `pending` and `inactive` -- so an admin had no way to filter that table down to pending inquiries at all, even though the per-row badge (driven by dynamic text, not a fixed option list) correctly showed "Pending". Lower severity than the two record-editors fixed below since a filter can't corrupt data on save, but same root cause. Added the two missing options.
- Verified live: selecting "Pending" in that filter now correctly shows inquiry 479 (previously the option didn't exist to select at all).

## 2026-09-03 — Fixed: two admin inquiry-status dropdowns silently flipped "pending" to "active" on save
**Files:** `app/Views/dashboard/admin/inquiry-form.php`, `app/Views/admin/settings/listings.php`
**Why:** found by the owner during front-end moderation testing -- the same pending inquiry (id 479) showed "Active" in both the admin's "Edit Buyer Inquiry" form and the Site Settings > Listings inquiries table, but "Pending" (correctly) in the Manage Buyer Inquiries table. DB-verified the real value was genuinely `pending` (`status=[pending] len=7`, no whitespace/encoding issue) -- so the two "Active"-showing views were the ones rendering wrong, not the table.
- Root cause: both `<select name="status">` elements only listed a subset of the real status values -- the edit form had `active`/`closed`/`expired` (no `pending`, no `inactive`); the Listings tab had only `active`/`inactive` (no `pending`/`closed`/`expired`). When a select's current value doesn't match any `<option>`, no `option` gets `selected` and the browser silently falls back to displaying the first one, `active` -- a pure rendering artifact, not a data change, but a dangerous-looking one.
- This was worse than a display bug: `Dashboard::editInquiry()` writes `'status' => $this->request->getPost('status') ?: 'active'` straight from the post body with no whitelist check. Since the dropdown was showing "Active" (though the real value was "pending") and would submit that visible value on save, an admin editing any unrelated field on a pending inquiry (fixing a typo, say) and clicking Save would have silently published it, bypassing the moderation queue entirely.
- Fixed both `<option>` lists to the full, real status set (`active`, `pending`, `inactive`, `closed`, `expired`) -- matching `AdminSettings::listings()`'s own whitelist (`['active', 'inactive', 'closed', 'pending', 'expired']`), which was already correct; only the two `<select>` markups were incomplete. The product status dropdown in the same `listings.php` already listed `active`/`pending`/`inactive` correctly -- the inquiries one just never got the same treatment.
- Also added a "N inquiries pending review" banner to the Listings > Inquiries tab, mirroring the one Products already had -- the missing banner is likely why this pending inquiry went unnoticed until the owner spotted the status text mismatch directly.
- Verified live: both dropdowns now show "Pending" selected for inquiry 479, matching the DB and the Manage Buyer Inquiries table; re-submitted the edit form without touching Status and confirmed via DB read that it still says `pending` afterward (previously this exact action would have flipped it to `active`).

## 2026-09-03 — Content moderation audit: remaining findings fixed (except popup lead capture)
**Files:** `app/Helpers/moderation_helper.php`, `app/Controllers/Dashboard.php`, `app/Views/admin/settings/moderation.php`
**Why:** closes out finding #3 (inconsistent admin-form coverage) and #4 (substring vs. word-boundary keyword matching) from the same audit as the entry below. Finding #5 (popup lead capture has no check at all) is deliberately left open -- owner asked to leave it out of this pass.
- **#3, admin-form consistency:** `Dashboard::addInquiry()`/`editInquiry()` (admin's own "Add/Edit Buyer Inquiry" screens) were the one admin-entered form still running the public-submission filter, unlike `addSupplier()`/`editSupplier()`/etc, which never had it. Decided admin data entry should be uniformly trusted (same reasoning as every other admin form) and removed the check from both, with a comment explaining why so it doesn't get re-added by accident.
- **#4, word-boundary matching:** `check_restricted_keywords()`'s custom-keyword check used a raw `strpos()` substring match, so a short admin-entered keyword like `"spa"` would false-positive block "aerospace" or "spare parts". Switched it to the same `\b...\b` word-boundary `preg_match()` the built-in profanity list already used, so only whole-word (or whole-phrase, for multi-word keywords) matches trigger a block.
- Updated the Content Moderation settings page's help text to mention the word-boundary behavior, that profile edits are now covered too, and that admin-entered data is exempt.
- Verified live: `"aerospace industry"` and `"spare parts"` no longer block against a `"spa"` keyword, while standalone `"spa"` and the exact multi-character test keyword still do; created disposable admin and buyer test accounts (not real credentials) to confirm admin `Add/Edit Inquiry` now accepts a restricted keyword that used to be blocked (DB-verified), while the buyer's *own* `Add Inquiry` form -- untouched by this change -- still correctly blocks the same keyword. All test settings, accounts, and inquiries reverted/deleted afterward; confirmed all four moderation settings match their original baseline values.

## 2026-09-03 — Content moderation audit: fixed the profile-edit gap, deduplicated the filter
**Files:** `app/Helpers/moderation_helper.php` (new), `app/Config/Autoload.php`, `app/Controllers/Dashboard.php`, `app/Controllers/Contact.php`, `app/Controllers/Auth.php`
**Why:** audited the Content Moderation admin settings (Restricted Keywords, Profanity Filter, Auto-Approve Listings, Require Admin Review) end to end. Auto-Approve and Require Admin Review both worked correctly (verified live + DB-checked). The keyword/profanity check worked on product add/edit, inquiry add/edit, registration, and the contact form -- but `Dashboard::updateProfile()` and `Dashboard::supplierEditProfile()` (a supplier/buyer editing their own public profile -- `selling_products`, `company_introduction`, `buying_products`, `requirement`) had no check at all. Confirmed live: saved a restricted keyword and two built-in profanity words straight into a supplier's `selling_products`/`company_introduction` with no error, then found that exact supplier via the live public search for "scam". Root cause: `checkRestrictedKeywords()` was copy-pasted separately into `Dashboard.php`, `Contact.php`, and `Auth.php` -- easy for a new form to simply never get it pasted in, which is exactly what happened to the two profile-edit forms.
- Added `check_restricted_keywords()` to a new autoloaded `moderation_helper.php` (registered in `Config/Autoload.php` next to `inquiry`/`seo`/`search`) -- one implementation instead of three, same matching logic as before (substring match against the admin's custom keyword list, `\b...\b` word-boundary match against the built-in profanity list when enabled).
- Removed the three duplicated `checkRestrictedKeywords()` methods and repointed every `$this->checkRestrictedKeywords(...)` call (6 in `Dashboard.php`, 2 in `Contact.php`, 1 in `Auth.php`) to the shared helper; dropped `Contact.php`'s now-unused `SiteSettingModel` import.
- Added the missing check to both `Dashboard::updateProfile()` (checks name, company_name, and whichever of selling_products/buying_products+requirement applies to that user type) and `Dashboard::supplierEditProfile()` (selling_products, company_introduction).
- Verified live: both previously-open forms now correctly block the same restricted keyword/profanity content that was silently accepted before; a legitimate profile update still saves normally (no over-blocking); re-ran all four original working flows (registration, contact, product add, and the original DB-level auto-approve/require-review checks) after the refactor to confirm nothing regressed.
- Left the other three audit findings (inconsistent admin-form coverage, substring-vs-word-boundary matching on custom keywords, unfiltered popup lead capture) as open items -- not requested for this pass.

## 2026-08-23 — Fixed: new footer office addresses were rendering black
**Files:** `app/Views/partials/footer.php`
**Why:** the footer's white text color for this column comes from `.footer-upper-links li a { color: #fff; }` in `style.css` -- a rule that only targets `<a>` elements. The three static office addresses added in the entry below have no `<a>` wrapper (that was the point -- no links), so they fell through to the default black text instead of matching the rest of the footer.
- Added `class="text-white"` to each of the three `<li>` elements (the same Bootstrap utility class already used elsewhere in this footer, e.g. the column headings) rather than adding a new CSS rule for a one-off case.
- Verified live: computed `color` of the Nigeria office `<li>` and of an existing footer link (`.footer-upper-links li a`) both read `rgb(255, 255, 255)`.

## 2026-08-23 — Footer "Get In Touch": replaced the single clickable address with three static office addresses
**Files:** `app/Views/partials/footer.php`
**Why:** the footer's "Get In Touch" column showed one admin-configurable `contact_address` (defaulting to a Montana, USA address) wrapped in a link to `/contact` that served no purpose as a link -- owner wants plain, non-clickable text instead, listing all three real office locations.
- Replaced the single `<a href="contact">...</a>` address entry with three plain `<li>` blocks (Nigeria, UK, China offices) -- no anchor tags, no `site_settings`/`contact_address` dependency.
- Left the phone number `<li>` (still a functional `tel:` click-to-call, gated on `contact_phone` being set) untouched -- the request was specifically about the address link, not the phone entry.
- Verified live: fetched the homepage footer's "Get In Touch" column and confirmed it now renders exactly the three office addresses as plain text with zero `<a>` tags inside that column (`querySelectorAll('a')` on it returns an empty array).

## 2026-08-23 — Highlight color customized; hidden-match note reworded and recolored
**Files:** `public/assets/css/style.css`, `app/Views/pages/product.php`, `app/Views/pages/supplier.php`, `app/Views/pages/buyer-main.php`
**Why:** owner wanted the `<mark>` highlight in a specific brand color instead of the browser default yellow, the "appeared N times" note to match that same color, and the wording changed to "appeared N more times" (since the note is specifically about occurrences beyond what's already visibly highlighted).
- Added `mark { background-color: #bfff4fd9; color: inherit; padding: 0 1px; }` to `style.css` (loaded by both `layouts/main.php` and `layouts/inner.php`, so it applies sitewide, not just the three search pages -- there was no other `<mark>` usage on the site to conflict with).
- Changed the hidden-match note's inline `background:#fff3cd` to the same `#bfff4fd9` in all three views; also dropped `buyer-main.php`'s extra `color:#664d03` (a dark-amber tuned for the old yellow) since the new lighter-green background reads fine with the default text color, matching the other two views.
- Reworded all three notes from `"{keyword}" appeared N time(s) in this record.` to `"{keyword}" appeared N more time(s) in this record.`
- Verified live: `product/search/steel` now reads `"steel" appeared 1 more time in this record.` and `buyer/search/corn` reads `"corn" appeared 8 more times in this record.`, both with `style="background:#bfff4fd9; ..."`; confirmed the `<mark>` color computes to `rgba(191, 255, 79, 0.85)` -- the exact RGBA equivalent of `#bfff4fd9` -- once the browser's stylesheet cache picked up the change (a local browser-cache staleness during testing, not an app issue).

## 2026-08-23 — Keyword highlighting on search results
**Files:** `app/Helpers/search_helper.php` (new), `app/Config/Autoload.php`, `app/Views/pages/product.php`, `app/Views/pages/supplier.php`, `app/Views/pages/buyer-main.php`
**Why:** the three search pages (`product/search`, `supplier/search`, `buyer/search`) returned matches but gave no visual indication of where the searched term actually matched within each result.
- Added `search_helper.php` (new autoloaded helper, registered in `Config/Autoload.php` alongside the existing `inquiry`/`seo` helpers): `highlight_keyword($text, $keyword)` escapes `$text` and wraps every case-insensitive match in `<mark>` (matching is done on the escaped text against an escaped keyword, so a keyword like `AT&T` still matches the escaped `AT&amp;T` correctly, and an HTML/script-injection attempt in either the record or the search box is neutralized before matching, not after); `count_keyword_occurrences($text, $keyword)` is a plain case-insensitive substring count, used for matches inside a field that never reaches the page at all, or is only partially shown.
- Per record type, only the fields that were actually part of the `LIKE` search AND are rendered get highlighted: product `name`; supplier `company_name` (falling back to `name` -- and only `name`'s matches count toward the hidden-match note when `company_name` was the one shown) and `selling_products`; buyer inquiry `title` and the first 200 characters of `description`.
- Fields that were searched but never rendered (`description`/`specifications` on products, `name` on a supplier displayed under its `company_name`, `product_name` on inquiries), or matches that fall past the 200-char description truncation on inquiries, don't get silently dropped -- a `"{keyword}" appeared N times in this record.` note renders instead, using the exact wording requested.
- Verified live: `product/search/steel` and `supplier/search/trading` show `<mark>` around matches (confirmed case-insensitivity: `Trading`/`trading`/`TRADING` all matched); `buyer/search/corn` shows both `<mark>` on the title/description and "appeared N times" notes where the count exceeds what's visible; isolated a throwaway `spark` command to run `highlight_keyword()`/`count_keyword_occurrences()` directly against HTML-special-character and `<script>` payloads to confirm safe escaping before deleting the command.

## 2026-08-23 — Added pagination to /product (it had none at all)
**Files:** `app/Controllers/Product.php`, `app/Views/pages/product.php`
**Why:** flagged live -- `/product` (`Product::index()`, and `Product::bySupplier()`/`/product/supplier/{id}`) had no pagination whatsoever until now (`findAll()`/`getActiveProducts()` with no limit), unlike `Supplier::index()`/`Buyer::index()` which already paginated. Not part of the `search()` clean-URL work earlier today since this page isn't in the `parse_search_path()`/`build_search_path()` filter family (no keyword/category segments, just a plain listing plus an optional `?supplier=` filter) -- so it uses CI4's own query-string `->paginate(12, 'product')`/`$pager`, the same pattern `Supplier::index()` and `Buyer::index()` already use, not the clean-URL `build_search_pager()` from today's earlier entries.
- `pages/product.php`'s pager block (added earlier today for `search()`'s clean-URL pager) now checks `$searchPager` first, falling back to `$pager->links('product', 'default_full')` -- the same dual-branch pattern already used in `pages/supplier.php` and `pages/buyer-main.php` for the same reason (one shared view, two different pagination sources depending on which controller method rendered it).
- The "Showing X out of Y" line already added today needed no changes -- it's driven by `isset($resultsTotal)` regardless of source, so passing `'resultsTotal' => $this->productModel->pager->getTotal('product')` from both methods was enough.
- Verified live: `/product` now shows "Showing 12 results out of 124" with working `?page_product=N` pager links; confirmed page 2 returns a genuinely different set of product IDs than page 1 (not the redirect-drops-the-page-number bug from earlier today -- this page has no legacy `?q=...` branch to collide with, so that bug class doesn't apply here).

## 2026-08-23 — "Showing X out of Y" extended to every other paginated listing
**Files:** `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`, `app/Views/pages/supplier-category.php`, `app/Views/pages/supplier-country.php`
**Why:** live browser testing of the previous entry (on `/supplier-category`) surfaced that the "Showing X out of Y" line had only been added to the three `search()` pages -- `Supplier::index()/category()/country()` and `Buyer::index()` all paginate with CI4's own `->paginate()`/`$pager` (not the manual clean-URL pager from the earlier entries today) and had the same gap.
- Added `'resultsTotal' => $this->userModel->pager->getTotal('supplier')` (and the `$pager->getTotal('buyer')` equivalent for `Buyer::index()`) to all four -- `Pager::getTotal($group)` already has the count CI4 ran for pagination, no extra query needed.
- Added the same `isset($resultsTotal)` "Showing X out of Y" line to `pages/supplier-category.php` and `pages/supplier-country.php` (the two views that didn't already get it from an earlier entry); `pages/supplier.php`'s "All Suppliers" branch already had it from the `Supplier::index()`-via-`search()` fix earlier today, so it picked up `Supplier::index()`'s own pagination for free.
- Verified live: `/supplier-category` (12/154), `/supplier-category/{slug}` (2/2), `/supplier-country` (12/154), `/supplier-country/CN` (12/72), `/supplier` (12/154), `/buyers` (50/470) all now show the count; confirmed `/supplier-country` was the one still missing it before this fix (`get_page_text` returned no "Showing" line) and now shows it.

## 2026-08-23 — "Showing X results out of Y" on the three search pages
**Files:** `app/Controllers/Product.php`, `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`, `app/Views/pages/product.php`, `app/Views/pages/supplier.php`, `app/Views/pages/buyer-main.php`
**Why:** with pagination now capping each page at 12/12/50 results, there was no indication of how many results existed beyond the current page.
- Each `search()` now passes `resultsTotal` (the `countAllResults(false)` figure already computed for pagination) to its view; the "shown" count is just `count($products/$suppliers/$inquiries)` on that page, so it's exact even on a partial last page.
- `pages/supplier.php` needed the line in both of its result branches (search-results and "All Suppliers", since a no-keyword search renders through the latter -- see the earlier changelog entry on that split); `pages/product.php` and `pages/buyer-main.php` needed it once each. All three are gated on `isset($resultsTotal)`, so `Supplier::index()`/`category()`/`country()` and `Buyer::index()` (none of which set it) are unaffected.
- Verified live: product search page 1 reads "Showing 12 results out of 124", its last page reads "Showing 4 results out of 124" (the true remainder); supplier search matches the same pattern (12/154, 10/154 on the last page); a keyword search narrower than one page reads "Showing 5 results out of 5"; buyer inquiries search reads "Showing 50 results out of 470".

## 2026-08-23 — Search pagination: 301 to the canonical URL when the page segment gets clamped
**Files:** `app/Controllers/Product.php`, `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`
**Why:** the clean-URL pagination in the entry below clamped an invalid/out-of-range `page` segment and rendered the clamped page's content, but under the original URL -- `.../page/-1`, `.../page/0`, `.../page/abc` all silently rendered page 1's content while the address bar and canonical tag still said `page/-1` etc, and `.../page/9999` rendered the last page's content under a URL that looks like it should 404 or be empty. Search engines and users both see a URL that doesn't describe what's on it.
- Each `search()` now compares the raw `page` segment against the clamped value it resolved to; if they differ, it 301s to the URL for the actual page being rendered (same permanent-redirect convention as the legacy `?q=...` and slug redirects elsewhere in these controllers) instead of rendering in place.
- Invalid values (`-1`, `0`, `abc`, or no digits at all) redirect to page 1's canonical URL, which is the bare search URL with no `page` segment. Out-of-range values (`page/9999`) redirect to the actual last page.
- A valid, in-range page (`page/2` when there are 11 pages) is left alone -- no redirect, no wasted round trip.
- Verified live with `curl -w "%{http_code} %{redirect_url}"`: confirmed a real `301` (not the app-level clamping alone) for `page/-1`, `page/0`, `page/abc` -> bare search URL, and `page/9999` -> the true last page, on all three search pages; confirmed keyword + filters survive the redirect (`product/search/laptop/page/-1` -> `product/search/laptop`; `supplier/search/trading/membership/gold/page/abc` -> `supplier/search/trading/membership/gold`); confirmed valid pages (`page/1`, `page/2`) render without redirecting.

## 2026-08-23 — Search pagination converted to clean-URL path segments
**Files:** `app/Controllers/Product.php`, `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`, `app/Helpers/seo_helper.php`, `app/Views/partials/search-pager.php` (new), `app/Views/pages/product.php`, `app/Views/pages/supplier.php`, `app/Views/pages/buyer-main.php`
**Why:** the query-string pagination from the previous two entries (`?page_product=N`) didn't match this site's established convention for these three pages -- every other filter (`category`, `country`, `membership`, `date`) is already a clean path segment via `parse_search_path()`/`build_search_path()` (see `search_slug_encode()` and friends in `seo_helper.php`), so `?page_product=N` was the only query-string holdout, and it was also the root cause of the two bugs fixed in the entries below.
- Replaced CI4's built-in `->paginate()`/`$pager->links()` (query-string based) with manual `countAllResults(false)` + `findAll($perPage, $offset)`, matching the same pattern `UserModel::getLeads()` already uses for admin pagination.
- Added `build_search_pager()` to `seo_helper.php`: builds pagination as `.../search/{keyword}/category/{slug}/page/{n}` using the existing `build_search_path()` helper, with `page` added as a `parse_search_path()` known key in all three controllers. Page 1 never emits a `/page/1` segment -- only one URL form for the first page.
- Added a shared `partials/search-pager.php` view rendering the same `<nav><ul class="pagination">` markup CI4's own `default_full` template used, so no new CSS was needed -- existing pagination styling picked it up unchanged.
- This also fully obsoletes the `page_product`/`page_supplier`/`page_buyer` query-exclusion patch from the entry below -- there's no pager query string left for that legacy `?q=...` redirect branch to misfire on, so that workaround was removed rather than left as dead code.
- Found and fixed a second bug while converting: `Supplier::search()` with no keyword renders through `pages/supplier.php`'s "All Suppliers" branch (keyed on `empty($searchKeyword)`), not its "search results" branch -- that branch only checked the old `isset($pager)`, so the new `$searchPager` never rendered anything until fixed.
- Verified live with direct edge-case testing: page `0`, negative, non-numeric, and far-beyond-range (`/page/9999`) all clamp correctly (page 0/negative/non-numeric → page 1, out-of-range → last page with its true remainder count); page 1 vs page 2 return genuinely different result sets for all three pages, browsing with and without a keyword; combined keyword+filter+page URLs (`/product/search/a/page/2`) carry every segment through correctly; the legacy `?q=...&filter=...` form still 301s to the correct clean URL with no stray `page` segment.

## 2026-08-23 — Fixed: no-keyword search pagination silently reset to page 1
**Files:** `app/Controllers/Product.php`, `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`
**Why:** the pagination added earlier today (previous entry below) turned out not to work when browsing without a keyword. Each `search()` method has a legacy branch that 301-redirects any `?q=...&category=...` style submission to the clean-URL form -- it triggers whenever there are no path segments and the query string is non-empty. A pager link on the bare browse view (no keyword, just `?page_product=2`) has no path segments either, so it was wrongly caught by that same branch and redirected back to the plain `/product/search` with the page number dropped -- every "page 2" click silently re-rendered page 1. A keyword search (`/product/search/laptop?page_product=2`) has a path segment, so it skipped this branch and was never affected -- which is why the initial verification (checking result counts on the keyword case) didn't catch it.
- Each `search()` now excludes its own pager query key (`page_product`/`page_supplier`/`page_buyer`) before deciding whether the request is an old-style query-string submission worth redirecting.
- Verified live via direct response diffing (not just visual/count checks this time): fetched page 1 vs page 2 HTML for all three no-keyword browse URLs and confirmed the returned product/supplier/inquiry IDs are now genuinely different sets; confirmed the legacy `?q=laptop` redirect still lands on `/product/search/laptop` unchanged; re-confirmed keyword-path pagination (`/product/search/laptop?page_product=2`) still works as it did before this fix.

## 2026-08-23 — Server-side pagination on Product/Supplier/Buyer search results
**Files:** `app/Controllers/Product.php`, `app/Controllers/Supplier.php`, `app/Controllers/Buyer.php`, `app/Views/pages/product.php`, `app/Views/pages/supplier.php`
**Why:** the three type-specific search pages (`product/search`, `supplier/search`, `buyer/search`) loaded every matching row via `findAll()` with no limit — unbounded query + full-page render on a broad search term.
- Switched all three `search()` methods from `findAll()` to the same `->paginate($perPage, $group)` convention already used by `Supplier::index()/category()/country()` and `Buyer::index()` (12/page for products and suppliers, 50/page for buyer inquiries, matching each page's existing non-search page size), and pass `$model->pager` to the view.
- `pages/buyer-main.php` already rendered `$pager` unconditionally for any state, so it picked up search pagination with no view change.
- Added a pager block to `pages/product.php` (had none before, for any listing) and to the search-results branch of `pages/supplier.php` (index/category/country already had one; search was the one branch missing it).
- The global multi-type quick search (`search/{keyword}`, capped at 6 results per type with "View All" links into these three pages) is deliberately unchanged — it's an overview, not a paginated listing.
- Verified live: `product/search`, `supplier/search`, and `buyer/search` each render 12/12/50 results with correct `page_product`/`page_supplier`/`page_buyer` pager links; paging to page 2 shows a different result set; a keyword search (`product/search/laptop`) still paginates correctly within the filtered result count.
