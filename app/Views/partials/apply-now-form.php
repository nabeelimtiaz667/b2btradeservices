...
#applyNowModal .form-input input,
#applyNowModal .form-input select {
    background: rgba(255,255,255,0.1);
    border: 1px solid rgba(255,255,255,0.3);
    color: #fff;
    border-radius: 6px;
...
#applyNowModal .form-input input::placeholder { color: rgba(255,255,255,0.55); }
#applyNowModal .form-input select option { color: #333; background: #fff; }
#applyNowModal .radio-join { color: rgba(255,255,255,0.85); font-size: 13px; }
#applyNowModal .whatsapp-checkbox { color: #fff; }
</style>

<div class="modal fade" id="applyNowModal" tabindex="-1" aria-labelledby="applyNowModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content" style="background: linear-gradient(135deg, #0a3d2e, #0F9EA5); border: none;">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title text-white" id="applyNowModalLabel">Apply Now</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body pt-2">
                <div id="applyFormSuccess" class="alert alert-success d-none">Thank you! Your application has been submitted. We will contact you shortly.</div>
                <div id="applyFormError" class="alert alert-danger d-none"></div>
                <form id="applyNowForm" action="<?= base_url('contact/submit-ajax') ?>" method="POST">
                    <?= csrf_field() ?>
                    <input type="hidden" name="form_type" value="<?= esc($formType ?? 'contact') ?>">
                    ...
                    <div class="whatsapp-checkbox">
                        <input type="checkbox" name="whatsapp" id="modal_whatsapp" value="1">
                        <label for="modal_whatsapp">Whatsapp <img src="<?= base_url('assets/images/whatsapp-icon.svg') ?>" width="15px"></label>
                    </div>
                    <div class="form-input">
                    ...
                    <div class="form-input selling-field">
                        <div class="multiselect-container">
                            <div class="multiselect-selected" id="multiselect-display" tabindex="0">
                                <span class="placeholder"><?= esc($placeholder ?? 'Select options...') ?></span>
                            </div>
                            <div class="multiselect-dropdown" id="multiselect-options">
                                <?php foreach (($options ?? []) as $opt): ?>
                                <div class="option" data-value="<?= esc($opt) ?>"><?= esc($opt) ?></div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div class="radio-join d-flex gap-2 align-items-start">
                        <input type="checkbox" name="OPT_IN" id="modal_OPT_IN" required>
                        <label for="modal_OPT_IN"><span style="color: #0F9EA5;">*</span>By joining. I agree to terms of use, privacy policy, IPR and agree to receive emails related to our services.</label>
                    </div>
                    <div class="submit-btn-gradient mt-3">
                        <button type="submit" id="applyFormBtn" class="gradeint-cta">Join Now</button>
                    </div>
                </form>
            </div>
        </div>
...
(function() {
    ...
    var selected = [];
    var placeholder = <?= json_encode($placeholder ?? 'Select options...') ?>;
    ...
    function renderTags() {
        ...
        dropdown.querySelectorAll('.option').forEach(function(opt) {
            opt.classList.toggle('selected', selected.indexOf(opt.dataset.value) > -1);
        });
    }

    display.addEventListener('click', function(e) {
        if (e.target.classList.contains('remove')) {
            selected = selected.filter(function(v) { return v !== e.target.dataset.val; });
            renderTags(); return;
        }
    ...
    dropdown.querySelectorAll('.option').forEach(function(opt) {
        opt.addEventListener('click', function() {
            ...
            idx > -1 ? selected.splice(idx, 1) : selected.push(val);
            renderTags();
        });
    });
    ...
    var modalEl = document.getElementById('applyNowModal');
    if (modalEl) {
    ...
    var formEl = document.getElementById('applyNowForm');
    if (formEl) {
        formEl.addEventListener('submit', function(e) {
            e.preventDefault();
            var btn = document.getElementById('applyFormBtn');
            var successDiv = document.getElementById('applyFormSuccess');
            var errorDiv   = document.getElementById('applyFormError');
            btn.disabled = true;
