...
class LeadCapture extends BaseController
{
    ...
    public function capture()
    {
        ...
        if ($this->leadModel->isConverted($existingLead)) {
            return $this->response->setJSON([
                'status'  => 'error',
                'code'    => 'already_user',
                'message' => 'This email already has an account. Please log in instead.',
        ...
        if ($this->leadModel->isNew($existingLead)) {
            $this->leadModel->reissueForNewLead($existingLead['id'], $data);
            $this->sendVerification($this->leadModel->find($existingLead['id']));
            return $this->response->setJSON(['status' => 'success', 'step' => 'check_email']);
        }

        // status = 'verified': this is the reactivation path -- a lead who
        // verified their email but never finished step 2 (or simply lost the
        // link) resubmits the popup to get it resent. The link itself never
        // changes here: it's still valid (expiry only ever applies to 'new',
        // see plan), so this re-sends the exact same URL rather than issuing
        // a new token. Cooldown is based on `updated_at` (captured before the
        // contact-details update below touches it), so the same form can't be
        // used to spam repeat emails at someone else's inbox.
    ...
    /**
     * Step 1 -> step 2 handoff. See plan's "Status lifecycle" table: expiry only
     * ever gates the 'new' -> 'verified' transition; once verified, this link
     * has no expiry and always re-opens step 2.
     */
    public function verify(string $token = '')
    {
        ...
        if ($this->userModel->findByEmail($lead['email'])) {
            if (!$this->leadModel->isConverted($lead)) {
                $this->leadModel->markConverted($lead['id']);
            }
        ...
        if ($this->leadModel->isNew($lead)) {
            ...
            $this->leadModel->markVerified($lead['id']);
        }
     ...
     * 'verified'. Combines the lead's captured name/type/email/phone with this
     * step's password/company/country/products into a single `users` insert,
     * then runs the same post-registration sequence Auth::register() uses.
     */
    public function completeSignup(string $token = '')
    {
        $lead = $this->leadModel->findByToken($token);

        if (!$lead || !$this->leadModel->isVerified($lead)) {
